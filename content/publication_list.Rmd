...
books <-
  c(
    ...
    # ToRecord(
    #   "03",
    #   "人工光環境と植物の光合成 (Artificial light environments and photosynthesis of plants)",
    #   paste(correspond(YI_), KM_, sep = ", "),
    #   "2018 (in press)",
    #   "Book chapter",
    #   "アグリフォトニクスIII",
    #   "")
...
symp_int <-
  c(  
    ToRecord(
      "05",
      new_item("Time course of leaf content of cytochrome *b*<sub>6</sub>*f* complex and photosynthetic capacity after changes in growth irradiance"),
      paste(speaker(KM), "Hui Zhu", "Ling-Da Zeng", "Xiao-Ping Yi", "Chang-Lian Peng", "Wang-Feng Zhang", WSC, sep = ", "),
      "2018",
      "Poster",
      "ComBio2018.<br>Sydney, Australia, 2018/09/23-2018/09/26",
      ""),
...
symposium_location_domestic <-
  ...
  map2(.x = .,
       ...
       function(x, y){
         geos <- geocode(y)
         tibble(name = x, lat = geos$lat, lng = geos$lon)
       }) %>%
  bind_rows()

symposium_location_int <-
  symp_int %>%
...
fill_na_recursive <-
  function(tbl, venue){
    na_index <- is.na(tbl$lat)
      
    for(xxx in which(na_index)){
      geo <- geocode(venue[xxx])
      tbl[xxx, "lat"] <- geo$lat
      tbl[xxx, "lng"] <- geo$lon
    }
    
    if(sum(is.na(tbl$lat)) != 0){
      fill_na_recursive(tbl, venue)
    } else {
      return(tbl)
    }
  }
...
symposium_location_domestic <-
  fill_na_recursive(tbl = symposium_location_domestic, venue = venue_domestic)
symposium_location_int <-
  fill_na_recursive(tbl = symposium_location_int, venue = venue_int)
...
#
geos_univ <- list(UT = c(139.762, 35.71268),
                  YU = c(131.4691, 34.14923),
                  ANU = c(149.1185, -35.2777))

flight_lines_ut <-
  map(3:5, function(xxx){
    xy_end <- c(symposium_location_int[xxx,]$lng, symposium_location_int[xxx,]$lat)
    
    gcIntermediate(geos_univ$UT, xy_end, n = 300, addStartEnd = T)
    })

flight_lines_anu <-
  map(1:2, function(xxx){
    xy_end <- c(symposium_location_int[xxx,]$lng, symposium_location_int[xxx,]$lat)
    
    gcIntermediate(geos_univ$ANU, xy_end, n = 300, addStartEnd = T)
    })

color_set_map <-
  list(domestic = "<FONT color='pink'>Domestic</FONT>",
       int = "<FONT color='skyblue'>International</FONT>")

color_flight_path <- "white"

symposium_map <-
  symposium_location_domestic %>%
  leaflet %>%
  addProviderTiles(providers$CartoDB.DarkMatter) %>%
  addPolylines(data = flight_lines_ut[[1]], color = color_flight_path, group = color_set_map[[2]]) %>%
  addPolylines(data = flight_lines_ut[[2]], color = color_flight_path, group = color_set_map[[2]]) %>%
  addPolylines(data = flight_lines_ut[[3]], color = color_flight_path, group = color_set_map[[2]]) %>%
  addPolylines(data = flight_lines_anu[[1]], color = color_flight_path, group = color_set_map[[2]]) %>%
  addPolylines(data = flight_lines_anu[[2]], color = color_flight_path, group = color_set_map[[2]]) %>%
  addCircleMarkers(lng = ~lng, lat = ~lat, popup = ~name, group = color_set_map[[1]], col = "pink", opacity = .9) %>%
  addCircleMarkers(data = symposium_location_int,
                   lng = ~lng, lat = ~lat, popup = ~name, group = color_set_map[[2]], col = "skyblue", opacity = .9) %>%
  addLayersControl(
    overlayGroups = c(color_set_map[[1]], color_set_map[[2]]),
