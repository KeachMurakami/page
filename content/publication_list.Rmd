...
```{r map, echo = F, cache = T, message = F, warning = F, out.width="100%"}
work_location <-
  data.frame(
    name = c("東大環境研", "山口大荊木研"),
...
symposium_map <-
  symposium_location_domestic %>%
    ...
    addCircleMarkers(col = "pink", lng = ~lng, lat = ~lat, popup = ~name, group = "Domestic") %>%
    addCircleMarkers(data = symposium_location_int, 
                     col = "skyblue", lng = ~lng, lat = ~lat, popup = ~name, group = "International") %>%
    addLayersControl(
      overlayGroups = c("Domestic", "International"),
      options = layersControlOptions(collapsed = FALSE)
    )

frameWidget(symposium_map)

# currentWD <- getwd()
# setwd("static/leaflet")
# saveWidget(symposium_map, "symposium_map.html")
# setwd(currentWD)
```
