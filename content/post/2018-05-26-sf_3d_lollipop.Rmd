---
title: "3D lollipop x map なアニメーションを (R"
date: 2018-05-26
---
...
<!-- `r htmltools::HTML("{{< tweet 994845385326809089 >}}")` -->
...
<iframe seamless src="../../../../plotly/lollipop_on_map4/index.html" width="100%" height="900"></iframe>
...
pop_geo_data <-
  ...
  {
    # 図示用に昼夜をずらす
    day_data = mutate(., day_night = "day", lng = lng - 0.01)
    night_data = mutate(., day_night = "night", lng = lng + 0.01)
    bind_rows(day_data, night_data)
  }
```
...
setwd("../plotly")
htmlwidgets::saveWidget(fig_scat, "lollipop_on_map1.html", selfcontained = T)
setwd("../post")
```

<iframe seamless src="../../../../plotly/lollipop_on_map1/index.html" width="100%" height="900"></iframe>
...
```{r}
for_droplines <- 
  replicate(2, pop_geo_data, simplify = F) %>%
...
setwd("../plotly")
htmlwidgets::saveWidget(fig_lollipop, "lollipop_on_map2.html", selfcontained = T)
setwd("../post")
```

<iframe seamless src="../../../../plotly/lollipop_on_map2/index.html" width="100%" height="900"></iframe>
...
## (閉じた区画ごとの座標データがnestしている？; documentにあたるべし) ので、
## map用に場合わけで対処する関数を作成
...
add_sf <-
  function(p, i){
    df <-
      ...
      filter(between(V1, 138.85, 140), between(V2, 35.3, 36))
    if(nrow(df) == 0){
      return(p)
    } else {
...
setwd("../plotly")
htmlwidgets::saveWidget(fig_lollipop_on_map, "lollipop_on_map3.html", selfcontained = T)
setwd("../post")
```

一通り完成。

<iframe seamless src="../../../../plotly/lollipop_on_map3/index.html" width="100%" height="900"></iframe>    

## グラフの微調整

layout(scene = ...)で見た目をいじる
...
scene <-
  list(
    xaxis = axis_set, yaxis = axis_set, zaxis = c(axis_set, backgroundcolor="rgb(100, 100, 100)", showbackground = T, range = list(c(0, 1200000))),
...
setwd("../plotly")
htmlwidgets::saveWidget(fig_lollipop_on_map_final, "lollipop_on_map4.html", selfcontained = T)
setwd("../post")
```
...
[時間によって激変するマンハッタンの人口を表す、インタラクティブな3Dグラフ | GIZMODE](https://www.gizmodo.jp/2018/05/manhattan-graph.html)  
